export default function Button({ type, name, value, disabled = null, onClick = null }) {
    return (
        <button className="w-full h-12 my-1 py-2 px-4 text-white rounded-xl transition-all bg-primary hover:bg-primaryHover active:bg-primaryActive active:text-white disabled:bg-stone-600"
            type={type}
            name={name}
            data-value={value}
            disabled={disabled}
